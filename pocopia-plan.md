...
| **개발 상태** | ✅ **완료** - 로컬 서버, 프론트엔드, 도메인 연결 완료 |
...
- [x] 입력 유효성 검증
  - 제목: 2~50자
  - 설명: 0~200자 (선택)
  - 코드: 정확히 8자리, Z/I/O 제외한 알파벳 대문자 + 숫자
- [x] 정적 파일 서빙

### 2.2 프론트엔드
- [x] index.html — 페이지 구조
- [x] style.css — 반응형 스타일링 (모바일/태블릿/데스크톱)
- [x] app.js — API 연동, 폼 제출, 실시간 목록, 카운트다운
- [x] i18n.js — 다국어 지원 (한국어/English/日本語)
- [x] 입력 실시간 차단 및 피드백
  - Z/I/O/소문자/특수문자/한글 입력 차단
  - 한글/특수문자 입력 시 시각적 힌트 (빨간 테두리 + 문구)
  - 자동 대문자 변환
...
### 3.1 도메인 및 외부 접근 ✅ 완료
- [x] `pokoclouds.com` 도메인 구매
- [x] Cloudflare 계정 생성 및 도메인 등록
- [x] Cloudflare Tunnel 설치 및 설정
- [x] `https://pokoclouds.com` 로컬 서버 연결
...
island = {
    ...
    "title": "string",              # 제목 (2~50자)
    "description": "string",        # 설명 (최대 200자, 선택)
    "code": "string",               # 8자리 클라우드섬 코드 (Z/I/O 제외한 알파벳 대문자 + 숫자)
...
- `code`: 필수, 정확히 8자리 (Z, I, O를 제외한 알파벳 대문자 + 숫자)
...
| 2026-07-01 | 1.2 | 모바일 반응형 개선, 입력 차단, 다국어 지원 (ko/en/ja), Z/I/O 코드 제외 |
